...
export const register = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!nome || !email || !password || !confirmPassword) {
    throw new BadRequestError({
      message: 'Verifique se os campos nome, email, password e confirmPassword foram preenchidos.',
    });
  }
  ...
  if (userExists) {
    throw new BadRequestError({ message: 'Email já cadastrado.' });
  }
...
export const verificarCodigo = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (user.verificado) {
    throw new BadRequestError({ message: 'Usuário já verificado.' });
  }
  ...
  await user.save();
  return successResponse(res, 200, 'E-mail verificado com sucesso!');
};
...
export const reenviarCodigo = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!email) {
    throw new BadRequestError({ message: 'Email é obrigatório.' });
  }
...
export const login = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!isMatch) {
    throw new UnauthorizedError({ message: 'Senha incorreta.' });
  }
...
export const refreshToken = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (error.name === 'TokenExpiredError') {
      throw new ForbiddenError({ message: 'Token expirado.' });
    }
    throw error;
  }
};
...
export const logout = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!user) {
    throw new NotFoundError({ message: 'Token inválido.' });
  }
...
export const resetPassword = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!user) {
    throw new BadRequestError({ message: 'Token inválido ou expirado' });
  }
